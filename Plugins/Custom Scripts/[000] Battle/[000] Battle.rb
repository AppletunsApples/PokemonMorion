...
class Battle::Move
  ...
  alias_method :bonemuscle_pbCalcTypeModSingle, :pbCalcTypeModSingle unless method_defined?(:bonemuscle_pbCalcTypeModSingle)
  def pbCalcTypeModSingle(moveType, defType, user, target)
    ret = bonemuscle_pbCalcTypeModSingle(moveType, defType, user, target) 

    if user.hasActiveAbility?(:BONEMUSCLE) && boneMove? && Effectiveness.ineffective_type?(moveType, defType)
...
class Battle::Move
  def pbCalcDamageMultipliers(user, target, numTargets, type, baseDmg, multipliers)
    ...
    #This is the only change, it is seperate from the only above so aura break wouldn't cancel this ability
    if (@battle.pbCheckGlobalAbility(:CUTENESSPROXIMITY) && type == :FAIRY)
      multipliers[:power_multiplier] *= 1.15
    end
